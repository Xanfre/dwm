...
monocle(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
		resize(c, m->wx + g, m->wy + g, m->ww - 2 * c->bw - 2 * g, m->wh - 2 * c->bw - 2 * g, 0);
}
...
movemouse(const Arg *arg)
{
	...
	do {
		...
		case MapRequest:
			...
			break;
		case MotionNotify:
			if ((ev.xmotion.time - lasttime) <= (1000 / REFRATE))
				continue;
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	if (!c->isfullscreen && !c->isfloating && !c->mon->enablegaps
...
tile(Monitor *m)
{
	...
	sx = mx = m->wx + g;
	sy = my = m->wy + g;
	mh = m->wh - 2*g - g * (MIN(n, m->nmaster) - 1);
	sh = m->wh - 2*g - g * (n - m->nmaster - 1);
	sw = mw = m->ww - 2*g;
